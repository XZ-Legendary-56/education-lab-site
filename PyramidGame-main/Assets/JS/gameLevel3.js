        ...
        if (isCorrect) {
            ...
            if (currentStage < 3) {
                ...
                setTimeout(() => {
                    ...
                    resetStage(); // Сбрасываем этап
                    // УВЕЛИЧЕНИЕ СКОРОСТИ ПАДЕНИЯ: Уменьшаем fallDuration на 1500 мс, но не ниже 1000 мс
                    config.fallDuration = Math.max(1000, config.fallDuration - 1500);
                    config.spawnInterval = Math.max(500, config.spawnInterval - 500);
